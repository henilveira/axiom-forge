...
- 2026-08-26 · apps/backend/eslint.config.mjs · `import/no-cycle` passa em silêncio (falso verde) em projeto TS sem `import/extensions`+`import/parsers` — o plugin ignora arquivos `.ts` ao percorrer o grafo · espalhe `...importPlugin.flatConfigs.typescript.settings` no bloco e valide com um ciclo proposital antes de confiar no gate
- 2026-08-26 · apps/backend/tsconfig.json · alias `@` apontando para `src/index.ts` (com extensão) faz `nest build` emitir `require("./index.ts")` e o `dist/` não sobe · aponte para `src/index` sem extensão
- 2026-08-26 · apps/backend/tsconfig.json · `nest build` reescreve o `tsconfig.json` (injeta `baseUrl` e remove comentários) · não use comentário lá e confira `git diff tsconfig.json` depois de buildar
