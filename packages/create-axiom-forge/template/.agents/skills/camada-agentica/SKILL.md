...
Consulte também `docs/engineering/agent-performance-review.md`,
`docs/engineering/tech-lead-decision-model.md`, `docs/engineering/git-flow-policy.md` e
`docs/engineering/github-pr-policy.md` para esta manutenção.
...
`security-reviewer`, `release-engineer` e `git-flow-specialist`.

`visual-first` é um modo de skill do frontend, não um owner adicional: mantém
`frontend-engineer` e `frontend-ui-engineer` como executores e apenas muda o
contrato para `PROPOSED` sem integração backend imediata.
...
- Cada skill descreve entrada, sequência, proibições, evidência e handoff; pré-voo e eficiência ficam
  no contrato central. Claude declara `model: sonnet`; Opus é proibido.
